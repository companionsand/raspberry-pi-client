...
def _authenticate_device(pairing_code: Optional[str] = None) -> Optional[Dict[str, Any]]:
    """
            ...
            if pairing_code:
                ...
                if serial_number:
                    if _pair_device(pairing_code, serial_number, orchestrator_base_url):
                        ...
                        return _authenticate_device(pairing_code=None)
                    else:
...
def authenticate(pairing_code: Optional[str] = None) -> Optional[Dict[str, Any]]:
    ...
    auth_result = _authenticate_device(pairing_code=pairing_code)
