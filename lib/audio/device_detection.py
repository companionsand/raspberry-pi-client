...
# Capture all 6 channels (always - works for both hardware AEC and WebRTC AEC)
# This uses dsnoop for multi-client capture support (like dmix for playback)
pcm.respeaker_6ch {{
    type dsnoop
    ipc_key {card_number}001
    slave {{
...
# Use respeaker_6ch for capture (all 6 channels via dsnoop for multi-client support)
# This works for both hardware AEC (uses Ch0) and WebRTC AEC (uses Ch0 + Ch5)
# dsnoop allows multiple clients to capture simultaneously, just like dmix for playback
pcm.!default {{
    type asym
    playback.pcm "respeaker_out"
    capture.pcm "respeaker_6ch"  # Always use 6-channel capture (extract Ch0 or Ch0+Ch5 as needed)
}}
...
def get_alsa_card_number(device_name: str) -> int:
    ...
    try:
        ...
        print(f"  ⚠ Could not determine ALSA card number: {e}")
        return None
    ...
    try:
        ...
        if check_result.returncode != 0:
            ...
            try:
                ...
                print(f"  ⚠ Could not initialize softvol: {init_error}")
            ...
            if check_result.returncode != 0:
                print(f"  ⚠ Softvol control still not available (may initialize on first audio playback)")
                if logger:
                    logger.warning(
        ...
        if result.returncode != 0:
            print(f"  ⚠ Failed to set softvol: {result.stderr.strip()}")
            if logger:
                logger.warning(
                    "softvol_set_failed",
                    extra={
        ...
        print(f"  ✓ Softvol set to {volume_percent}%")
        if logger:
            logger.info(
                "softvol_set",
                extra={
        ...
        print(f"  ⚠ Timeout setting softvol")
        if logger:
            logger.error(
                "softvol_timeout",
                extra={
        ...
        print(f"  ⚠ amixer command not found (install alsa-utils)")
        return False
    except Exception as e:
        print(f"  ⚠ Error setting softvol: {e}")
        if logger:
            logger.error(
                "softvol_error",
                extra={
...
def ensure_alsa_config(card_number: int):
    ...
    try:
        ...
        if os.path.exists(asound_conf_path):
            ...
            has_respeaker_6ch = 'respeaker_6ch' in existing_config.lower()
            ...
            if has_softvol and has_respeaker_6ch and has_default_pcm and has_correct_card:
                # Config looks good
                print(f"✓ /etc/asound.conf: Already configured for card {card_number} with softvol")
                if logger:
                    logger.info(
                ...
                if not has_correct_card and has_softvol:
                    print(f"⚠ /etc/asound.conf: Configured for wrong card, updating to card {card_number}...")
                else:
            ...
            print(f"⚠ /etc/asound.conf: Not found, creating for card {card_number} with softvol...")
        ...
        print(f"✗ /etc/asound.conf: Error configuring: {e}")
        if logger:
            logger.error(
                "alsa_config_error",
                extra={
...
def get_audio_devices():
    ...
    if respeaker_found:
        ...
        if alsa_card_number is None:
            ...
            print(f"  ⚠ Could not detect ALSA card number, using sounddevice index {respeaker_info['index']}")
        ...
        print(f"✓ ReSpeaker detected: {respeaker_info['name']}")
        ...
        print(f"  ✓ Using Channel 0 (AEC-processed audio) from ReSpeaker")
        print(f"  ✓ Software volume control (softvol) enabled")
