...
class Tuning:
    ...
    def __init__(self, dev, detached_interfaces=None):
        self.dev = dev
        self.detached_interfaces = detached_interfaces or []
    ...
    def close(self):
        """
        close the interface and re-attach kernel drivers if they were detached
        """
        usb.util.dispose_resources(self.dev)
        for i in self.detached_interfaces:
            try:
                self.dev.attach_kernel_driver(i)
                # print(f"Re-attached kernel driver for interface {i}")
            except Exception as e:
                # print(f"Failed to re-attach kernel driver for interface {i}: {e}")
                pass
...
def find(vid=0x2886, pid=0x0018):
    ...
    # Detach kernel driver if active to allow libusb control
    # This fixes [Errno 5] Input/Output Error on Linux
    detached = []
    for i in range(5):
        try:
            if dev.is_kernel_driver_active(i):
                # print(f"Detaching kernel driver for interface {i}...")
                dev.detach_kernel_driver(i)
                detached.append(i)
                # print(f"Kernel driver detached for interface {i}.")
            else:
                pass
                # print(f"Kernel driver not active on interface {i}.")
        except Exception as e:
            pass
            # print(f"Error checking/detaching kernel driver for interface {i}: {e}")

    return Tuning(dev, detached_interfaces=detached)
